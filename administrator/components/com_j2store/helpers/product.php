...
class J2Product extends JObject{
	...
	*/
	...
	public function getPrice($variant, $quantity=1, $group_id='', $date='') {
		...
		J2Store::plugin()->event('AfterGetPrice', array(&$pricing));
			...
			if( $max && $max > 0 )
				{
					...
					{
						$error = JText::sprintf('J2STORE_MAX_QUANTITY_FOR_PRODUCT', floatval($max), $cart_total_qty);
					}
			}
		...
		if (! empty ( $upsell_csv )) {
			...
			if (count ( $up_sells )) {
				...
				foreach ( $up_sells as $upsell ) {
					...
					if(empty($upsell_product->product_name)) continue;
		...
		if (! empty ( $cross_sell_csv )) {
			...
			if (count ( $cross_sells )) {
				...
				foreach ( $cross_sells as $cross_sell ) {
					...
					if(empty($cross_sell_product->product_name)) continue;
