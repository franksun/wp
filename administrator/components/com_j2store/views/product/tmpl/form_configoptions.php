		...
		$(document).ready(function() {
			$('#optionselector').autocomplete({
				source : function(request, response) {
					$.ajax({
						type : 'post',
						url  :  'index.php?option=com_j2store&view=options&task=getOptions&product_type=<?php echo $this->item->product_type;?>',
						data :'q=' + request.term,
						dataType : 'json',
				...
				select : function(event, ui) {
					...
					$('<tr><td class=\"addedOption\">' + ui.item.label+ '</td><td></td>'
							...
							+'</td><td><span class=\"optionRemove\" onclick=\"j2store.jQuery(this).parent().parent().remove();\">x</span>'
