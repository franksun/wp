		...
		if($type != 'update') {
			...
			if (JFolder::exists($template_path.'/mod_j2store_products'))
			{
				if (JFolder::exists($template_path.'/old_mod_j2store_products'))
				{
					...
					{
						$parent->getParent()->abort('Could not delete folder '.$template_path.'/old_mod_j2store_products. Check permissions.');
						return false;
					}
				}
				if (!JFolder::move($template_path.'/mod_j2store_products', $template_path.'/old_mod_j2store_products'))
				{
					$parent->getParent()->abort('Could not move folder '.$template_path.'/mod_j2store_products. Check permissions.');
					return false;
				}
			...
			if (JFolder::exists($template_path.'/mod_j2store_detailcart'))
			{
				if (JFolder::exists($template_path.'/old_mod_j2store_detailcart'))
				{
					...
					{
						$parent->getParent()->abort('Could not delete folder '.$template_path.'/old_mod_j2store_detailcart. Check permissions.');
						return false;
					}
				}
				if (!JFolder::move($template_path.'/mod_j2store_detailcart', $template_path.'/old_mod_j2store_detailcart'))
				{
					$parent->getParent()->abort('Could not move folder '.$template_path.'/mod_j2store_detailcart. Check permissions.');
					return false;
				}
			...
			if (JFolder::exists($template_path.'/mod_j2store_cart'))
			{
				if (JFolder::exists($template_path.'/old_mod_j2store_cart'))
				{
					...
					{
						$parent->getParent()->abort('Could not delete folder '.$template_path.'/old_mod_j2store_cart. Check permissions.');
						return false;
					}
				}
				if (!JFolder::move($template_path.'/mod_j2store_cart', $template_path.'/old_mod_j2store_cart'))
				{
					$parent->getParent()->abort('Could not move folder '.$template_path.'/mod_j2store_cart. Check permissions.');
					return false;
				}
