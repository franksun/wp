...
class J2StoreModelCartsBehaviorCartSimple extends F0FModelBehavior {
	...
	public function onGetCartItems(&$model, &$item) {
		...
		//echo $item->weight = $item->weight + $product_option_data['option_weight'];
		$item->weight_total = ($item->weight + $product_option_data['option_weight']) * $item->product_qty;

		$item->pricing = $product_helper->getPrice($item, $item->product_qty);
