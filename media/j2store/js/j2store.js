...
function doAjaxFilter(pov_id, product_id, po_id, id) {
	(function($) {
		...
		$.ajax({
					url : 'index.php?option=com_j2store&view=products&task=update&po_id='
							+ po_id
							+ '&pov_id='
							+ pov_id
							+ '&product_id='
					...
					dataType : 'json',
					beforeSend: function() {
						$('#option-' + po_id).append('<span class="wait">&nbsp;<img src="/media/j2store/images/loader.gif" alt="" /></span>');
					},
					complete: function() {
						$('.wait').remove();
					},
...
function doAjaxPrice(product_id, id) {
	(function($) {
		...
		$.ajax({
			url : 'index.php?option=com_j2store&view=product&task=update',
			type : 'post',
			data : values,
			dataType : 'json',
			success : function(response) {
...
function resetJ2storeProductFilter(productfilter_class,inputid){
	...
	//call the method to load loading image
	//submit the form
	getJ2storeFiltersSubmit();
}
